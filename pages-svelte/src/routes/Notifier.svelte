<script>
    ...
    try {
      ...
      if (saved) {
        ...
        if (data.lastSaved && data.lastSaved > sevenDaysAgo) {
          ...
          console.log(`[Storage] Loaded ${knownFollowers.size} known followers from storage (루블리스 excluded)`);
          console.log(`[Storage] Original count: ${data.followers.length}, After excluding 루블리스: ${followersWithoutRublis.length}`);
  ...
  onMount(async () => {
    ...
    console.log(
      "[Svelte] App Initialized at",
      new Date(appStartedAt).toISOString(),
    );
    ...
    const isOBSMode = window.OBS_MODE || (!api.isTauri && (window.location.pathname === '/follower' || window.location.hash === '#/notifier'));
    
    // Environment-specific initialization
    if (!api.isTauri || isOBSMode) {
      console.log("[init] OBS-MODE detected - enabling OBS functionality");
      document.body.classList.add('obs-mode');
      
      // OBS 모드에서는 기본 포트 사용
      baseUrl = window.location.origin;
      obsUrl = `${baseUrl}/follower`;
      console.log("[Notifier] OBS mode - Base URL:", baseUrl);
    } else {
      console.log("[init] Tauri-MODE detected - enabling full functionality");
      ...
      try {
        ...
        console.log("[Notifier] Base URL set to:", baseUrl);
        console.log("[Notifier] OBS URL set to:", obsUrl);
      } catch (portError) {
        console.error("[Notifier] Failed to get dynamic port:", portError);
        // Keep default port as fallback
        console.log("[Notifier] Using fallback Base URL:", baseUrl);
      }
    ...
    try {
      ...
      if (removedCount > 0) {
        console.log(`[init] 루블리스 forcefully removed from known followers (${removedCount} entries)`);
        saveKnownFollowers(); // 즉시 저장
      } else {
        console.log("[init] 루블리스 not found in known followers - good!");
      }
      
      console.log(`[init] Known followers loaded successfully. Final count: ${knownFollowers.size}`);
  ...
  async function loadSettings() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (serverSettings.enableTTS !== undefined)
          enableTTS = serverSettings.enableTTS;
        ...
        if (serverSettings.textColor !== undefined)
          textColor = serverSettings.textColor;
        if (serverSettings.textSize !== undefined)
          textSize = serverSettings.textSize;
      } else {
      ...
      try {
        ...
        if (s.getItem("displayDuration"))
          displayDuration = parseInt(s.getItem("displayDuration"));
        if (s.getItem("enableTTS"))
          enableTTS = s.getItem("enableTTS") === "true";
        if (s.getItem("customSoundPath"))
          customSoundPath = s.getItem("customSoundPath");
        if (s.getItem("animationType"))
          animationType = s.getItem("animationType");
        if (s.getItem("notificationLayout"))
          notificationLayout = s.getItem("notificationLayout");
        if (s.getItem("textColor"))
          textColor = s.getItem("textColor");
        if (s.getItem("textSize"))
          textSize = parseInt(s.getItem("textSize"));
    ...
    console.log("[Settings] Final settings:", {
      volume, pollingInterval, displayDuration, enableTTS, 
      customSoundPath, animationType, notificationLayout, textColor, textSize
    });
  }
  ...
  function initializeWebSocket() {
    ...
    try {
      ...
      wsClient.on('connected', () => {
        ...
        // WebSocket 연결 시 폴링 중단
        pollingEnabled = false;
        if (pollingTimeoutId) {
          clearTimeout(pollingTimeoutId);
          pollingTimeoutId = null;
          console.log("[WebSocket] Polling disabled - using real-time connection");
        }
      });
  ...
  function handleNewFollowerFromWS(follower) {
    ...
    if (isTestFollower) {
      console.log("[WebSocket] Test follower (루블리스) - always show notification:", follower.user.nickname);
      ...
      if (follower.followingSince) {
        try {
          ...
          if (!isNaN(followTime) && followTime < appStartedAt) {
            console.log(`[WebSocket] Old follower detected (followed at ${follower.followingSince}, app started at ${new Date(appStartedAt).toISOString()}), adding to known set:`, follower.user.nickname);
          ...
          console.log("[WebSocket] Time parsing failed, treating as known follower:", follower.user.nickname);
          knownFollowers.add(follower.user.userIdHash);
          saveKnownFollowers();
          return;
        }
  ...
  function handleSettingsUpdateFromWS(settings) {
    ...
    if (!params.has("displayDuration") && settings.displayDuration !== undefined && displayDuration !== settings.displayDuration) {
      displayDuration = settings.displayDuration;
      settingsChanged = true;
      console.log("[WebSocket] Updated displayDuration:", displayDuration);
    }
    if (!params.has("animationType") && settings.animationType !== undefined && animationType !== settings.animationType) {
      animationType = settings.animationType;
      settingsChanged = true;
      console.log("[WebSocket] Updated animationType:", animationType);
    }
    if (!params.has("notificationLayout") && settings.notificationLayout !== undefined && notificationLayout !== settings.notificationLayout) {
      notificationLayout = settings.notificationLayout;
      settingsChanged = true;
      console.log("[WebSocket] Updated notificationLayout:", notificationLayout);
    }
    if (!params.has("textColor") && settings.textColor !== undefined && textColor !== settings.textColor) {
      textColor = settings.textColor;
      settingsChanged = true;
      console.log("[WebSocket] Updated textColor:", textColor);
    }
    if (!params.has("textSize") && settings.textSize !== undefined && textSize !== settings.textSize) {
      ...
      if (res.ok) {
        ...
        followers.forEach((f) => {
          ...
          if (f.user.nickname !== "루블리스") {
            ...
            console.log("[Init] 루블리스 excluded from known followers - will always show notifications");
        ...
        console.log(`[Init] Total followers found: ${followers.length}, Known followers: ${registeredCount}`);
        ...
        if (isOBSMode) {
          ...
          console.log("[Init] Tauri mode detected - 루블리스 will be processed via WebSocket and polling");
        }
      }
    } catch (e) {
  ...
  function scheduleNextPoll() {
    ...
    if (wsConnected || !pollingEnabled) {
      console.log("[Polling] Skipping poll - WebSocket connected or polling disabled");
      return;
    }
  ...
  async function pollLoop() {
    // WebSocket 연결된 경우 폴링 중단
    if (wsConnected || !pollingEnabled) {
      console.log("[Polling] Poll cancelled - WebSocket connected or polling disabled");
      return;
    }
  ...
  async function fetchFollowers() {
    ...
    try {
      ...
      if (newFollowers.length > 0) {
        ...
        console.log(`[Fetch] New followers list:`, newFollowers.map(f => f.user.nickname));
        ...
        if (validNewFollowers.length > 0) {
          ...
          console.log(`[Fetch] Queue contents:`, validNewFollowers.map(f => f.user.nickname));
  ...
  async function attemptReconnect() {
    console.log(`[Reconnect] Attempting reconnection ${reconnectAttempts + 1}/${maxReconnectAttempts}`);
    isReconnecting = true;
    reconnectAttempts++;
    ...
    try {
      ...
      if (reconnectAttempts >= maxReconnectAttempts) {
        ...
        console.log(`[Reconnect] Scheduling next attempt in 3 seconds (${reconnectAttempts}/${maxReconnectAttempts})`);
  ...
  function processQueue() {
    console.log(`[Queue] Processing queue. Items: ${queue.length}, Currently processing: ${isProcessing}`);
    ...
    if (queue.length > 0 && !isProcessing) {
      ...
      try {
        ...
        setTimeout(() => {
          console.log(`[Queue] Display duration expired for: ${currentItem?.user?.nickname || 'unknown'}`);
          currentItem = null;
  ...
  function startSettingsSync() {
        ...
        if (res.ok) {
          ...
          if (lastSettingsHash !== null && lastSettingsHash !== currentHash) {
            ...
            const settingsToUpdate = [
              ...
              { key: 'notificationLayout', param: 'notificationLayout', current: notificationLayout },
            ...
            settingsToUpdate.forEach(({ key, param, current }) => {
              if (!params.has(param) && serverSettings[key] !== undefined && current !== serverSettings[key]) {
                switch (key) {
                  case 'volume': volume = serverSettings[key]; break;
                  case 'displayDuration': displayDuration = serverSettings[key]; break;
                  case 'animationType': animationType = serverSettings[key]; break;
                  case 'notificationLayout': notificationLayout = serverSettings[key]; break;
                  case 'textColor': textColor = serverSettings[key]; break;
                  case 'textSize': textSize = serverSettings[key]; break;
                }
                settingsChanged = true;
    ...
    // 2초마다 설정 동기화 확인 (1초에서 2초로 변경하여 성능 개선)
    settingsSyncIntervalId = setInterval(syncSettings, 2000);
    console.log("[SettingsSync] Settings sync started (2s interval)");
  }
</script>
...
<div class="notifier-container">
  ...
  {#if showSettings}
    <div class="settings-modal">
      ...
      <div class="form-group">
        <label for="duration"
          >알림 표시 시간 ({displayDuration}초)</label
        >
        <input
          id="duration"
          type="range"
      ...
      <div class="form-group">
        ...
        <div class="file-select-group">
          <button class="btn btn-secondary" onclick={selectSoundFile}
            >파일 선택</button
          >
      ...
      >
        ...
        <div
          style="display:flex; align-items:center; background:#333; padding:5px; border-radius:4px; margin-bottom:10px;"
        >
