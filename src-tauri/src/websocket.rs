...
pub enum WSMessage {
    ...
    NewFollower {
        follower: crate::chzzk::FollowerItem,
    },
...
impl WSConnectionPool {
    ...
    pub async fn add_client(&self, client: WSClient) -> Result<(), String> {
        ...
        if clients.len() >= self.connection_limit {
            ...
            if let Some(oldest_id) = self.find_oldest_inactive_client(&clients).await {
                ...
                return Err(
                    "Connection limit reached and no inactive clients to remove".to_string()
                );
            }
        }
    ...
    pub async fn remove_client(&self, client_id: &str) {
        ...
        if clients.remove(client_id).is_some() {
            log::info!(
                "[WSPool] Client removed: {} (Remaining: {})",
                client_id,
                clients.len()
            );
        }
    ...
    pub async fn cleanup_stale_clients(&self) {
        ...
        if !clients.is_empty() {
            log::debug!(
                "[WSPool] Cleanup completed. Active clients: {}",
                clients.len()
            );
        }
    }
    ...
    // 가장 오래된 비활성 클라이언트 찾기
    async fn find_oldest_inactive_client(
        &self,
    ...
    pub async fn broadcast_new_follower(&self, follower: crate::chzzk::FollowerItem) {
        ...
        if let Err(e) = self.broadcast_tx.send(message) {
            ...
            log::info!(
                "[WSPool] Broadcasted new follower to {} clients",
                client_count
            );
        }
    }
    ...
    pub async fn broadcast_test_notification(&self, follower: crate::chzzk::FollowerItem) {
        ...
        if let Err(e) = self.broadcast_tx.send(message) {
            ...
            let client_count = self.client_count().await;
            log::info!(
                "[WSPool] Broadcasted test notification to {} clients",
                client_count
            );
        }
    }
    ...
    pub async fn broadcast_settings_update(&self, settings: serde_json::Value) {
        ...
        if let Err(e) = self.broadcast_tx.send(message) {
            ...
            let client_count = self.client_count().await;
            log::info!(
                "[WSPool] Broadcasted settings update to {} clients",
                client_count
            );
        }
    }
    ...
    pub fn start_cleanup_task(&self) {
        ...
        log::info!(
            "[WSPool] Cleanup task started (interval: {:?})",
            ...
            match global_rx.recv().await {
                Ok(ws_msg) => {
                    ...
                    if sender.send(Message::Text(json)).await.is_err() {
                        log::info!(
                            "[WebSocket] Client {} disconnected (send failed)",
                            client_id_clone
                        );
                        break;
                    }
                    ...
                    // 활동 업데이트
                    ws_manager_clone
                        .update_client_activity(&client_id_clone)
                        .await;
                }
        ...
        while let Some(msg) = receiver.next().await {
            match msg {
                Ok(Message::Text(text)) => {
                    ...
                    // 클라이언트 메시지 처리
                    if let Err(e) =
                        handle_client_message(&text, &client_id_clone, &ws_manager_clone).await
                    {
                        log::error!(
                            "[WebSocket] Error handling message from {}: {}",
                            client_id_clone,
                            e
                        );
                    }
                ...
                Ok(Message::Binary(_)) => {
                    log::debug!(
                        "[WebSocket] Received binary message from {} (ignored)",
                        client_id_clone
                    );
                }
                Err(e) => {
                    log::error!(
                        "[WebSocket] Error receiving message from {}: {}",
                        client_id_clone,
                        e
                    );
                    break;
...
async fn handle_client_message(
    ...
    match message {
        ...
        WSMessage::TestFollower => {
            log::info!(
                "[WebSocket] Test follower request from client {}",
                client_id
            );
        ...
        _ => {
            log::debug!(
                "[WebSocket] Unhandled message from client {}: {:?}",
                client_id,
                message
            );
        }
