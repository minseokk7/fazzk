...
export class ConnectionManager {
  ...
  startReconnect(): void {
    ...
    log.info(
      `Scheduling reconnect attempt ${this.state.reconnectAttempts}/${this.state.maxReconnectAttempts} in ${delay}ms`
    );
    ...
    if (!isOBSMode) {
      ...
      loadingManager.start(
        loadingId,
        `재연결 시도 중... (${this.state.reconnectAttempts}/${this.state.maxReconnectAttempts})`,
        {
          category: 'websocket',
          priority: 'high',
          cancellable: true,
  ...
  private executeReconnect(): void {
    ...
    window.dispatchEvent(
      new CustomEvent('connection:reconnect', {
        detail: {
